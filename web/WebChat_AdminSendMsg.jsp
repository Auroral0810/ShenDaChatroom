...
<head>
  ...
  <style>
    ...
    .submit-button, .logout-button, .maintenance-button {
      width: 150px;
      padding: 12px 20px;
      border: none;
      border-radius: 8px;
...
<%--  <script>
      ...
      // 允许的文件类型
      const allowedFileTypes = ['image/png', 'image/jpeg', 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'text/csv', 'text/plain', 'text/x-python'];

      if (file) {
      // 检查文件类型
      if (!allowedFileTypes.includes(file.type)) {
      alert('不支持的文件类型，请选择PNG、JPEG、PDF、Word、CSV、TXT或Python文件。');
      clearFileSelection();
      return;
    }

      selectedFile = file; // 保存文件到全局变量
      fileNameInput.value = file.name; // 将文件名显示在输入框中
      filePreview.style.display = 'flex'; // 显示文件预览容器

      // 调用 displayStatusMessage 显示成功消息
      var contextPath = '<%= request.getContextPath() %>';
      displayStatusMessage("文件上传成功 <img src='" + contextPath + "/image/success.png' alt='Success' class='upload-success-icon'>", "success");
    } else {
      clearFileSelection(); // 如果没有文件，清除预览
    }
    }
      ...
      filePreview.style.display = 'none'; // 隐藏预览容器
      selectedFile = null; // 清空全局文件变量
    }
    ...
    function validateForm() {
      ...
      const privateRecipientSelect = document.getElementById('privateRecipient');
  ...
  <script>
    ...
    function openMaintenance() {
      top.location.href = 'WebChat_Maintenance.jsp';
    }
...
<div class="chat-container">
  ...
  %>
  <h2 class="welcome-header">欢迎，管理员 <%= currentUser %>！</h2>
  <form id="messageForm" action="FileUploadServlet" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
    ...
    <div class="input-row">
      ...
      <div class="emoji-panel" id="emojiPanel">
        ...
        <div class="emoji-pagination">
          <button type="button" id="prevPageBtn" onclick="prevEmojiPage()" style="display:none;">上一页</button>
          <button type="button" id="nextPageBtn" onclick="nextEmojiPage()" style="display:none;">下一页</button>
        </div>
      </div>
    </div>
    <!-- 隐藏的文件选择框 -->
    ...
    <select id="privateRecipient" name="privateRecipient" class="private-recipient-select">
      ...
      <%
        ...
        try {
          ...
          ResultSet rs=stmt.executeQuery("SELECT username FROM webChat.dbo.Login WHERE username !='"+currentUser+"'");
          while (rs.next()) {
    ...
    <div class="button-container">
      ...
      <button type="button" class="maintenance-button" onclick="openMaintenance();">聊天室维护</button>
    </div>
  </form>
